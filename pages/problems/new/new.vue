			...
			<view class="problem-scroll-container">
				<scroll-view scroll-y="true" class="sv" :style="{height:navHeight+'px'}">
					<view class="problem-info-container">
						...
						<view class="info-container" style="overflow: hidden;">
							<view class="uni-list-cell-left">
								<text style="color: red;">*</text>
								问题细目
							</view>
						...
						<view class="info-container">
							...
							<view class="uni-list-cell-db">
								<el-select size="small" v-model="value_select" multiple filterable placeholder="请选择"
									style="width: 100%;margin-top: 5px;border: 1px solid #1E90FF;border-radius: 6px;"
...
<script>
	...
	export default {
		...
		methods: {
			...
			//选择图片
			select(e) {
				this.imageValue = this.imageValue.concat(e.tempFilePaths) //图片url
				...
				this.imageValue.map(data => { //图片转base64
					console.log(data)
			...
			bindPickerChange3: function(e) {
				...
				if (this.items[this.current] == "自查问题") {
					...
					getRole(this.stateUser.admin_info[0].role_name, this, this.stateUser.token).get_problem_locations(
						this.problem_categories1, this.current, this.stateUser.admin_info[0].department_uuid).then(
						() => {
							this.bindPickerChange1(1)
						})
			...
			onShow() {
				...
				} //如果登录单位为大队获取其所在支队uuid
				console.log(this.items[this.current])
				getRole(this.stateUser.admin_info[0].role_name, this, this.stateUser.token, this.items)
					.get_problem_locations(this
			...
			problemCreate() {
				...
				if (this.problem.file_content) {
					this.problemCreate_obj.file_content = this.problem.file_content
					this.problemCreate_obj.file_suffix = this.problem.file_suffix
				}
				if (this.problem.responsible_uuids) {
					this.problemCreate_obj.responsible_uuids = this.problem.responsible_uuids
				}
				this.postproblems()
