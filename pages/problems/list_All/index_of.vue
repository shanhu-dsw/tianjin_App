<template>
	<view style="background-color: #F1F1F1;">
		<view class="page-top">
			<view class="container-go-back" @click="backhome" hover-class="other-navigator-hover" open-type="reLaunch">
		...
		<view class="container">
			<view class="uni-list-cell-db" style="display: flex;margin-bottom: 5px">
				<input class="uni-list-cell-picker" type="text" placeholder="" placeholder="请输入问题编号"
					v-model="current_search_value" style="flex: 3;">
...
<script>
	...
	export default {
		...
		onLoad(options) {
			console.log(options)
			this.id = options.id
			this.value_time = options.division_time
			this.value_status = options.division_state
			this.value_type = options.division_type
		...
		methods: {
			...
			getproblems() {
				this.$api.getProblemExact.post(this.problem_data).then(response => {
					// if(this.stateUser.admin_info[0].role_name == "superadmin"&&"ds_admin"&&"ds_user"){
			...
			onShow() {
				...
				if (this.value_status == 1) {
					this.problem_data.status = "UN_FEEDBACK"
				} else if (this.value_status == 2) {
					this.problem_data.status = "FEEDBACK"
				} else if (this.value_status == 3) {
					this.problem_data.status = "FILED"
				}
