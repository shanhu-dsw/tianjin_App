<template>
	<view class="">
		<u-popup :show="show" @close="close" @open="open" :closeable="true">
			...
			<view class="info-container">
				<textarea class="uni-list-cell-textarea" placeholder="请输入整改说明" v-model="problem_state"></textarea>
			</view>
			<view class="info-container">
				<view class="uni-list-cell-left">
					上传整改图片
...
<script>
	...
	export default {
		props: ['show', 'id', "is_fact", "corrected_note", "Feedback_id"],
		data() {
			return {
				problem_state: "",
				imageValue: "",
		...
		methods: {
			onShow() {
				this.problem_state = this.id
				console.log(this.corrected_note)
				console.log(this.storage_url)
				console.log(this.is_fact)
			},
			open() {
			...
			updata_state() {
				...
				if(this.file_content){
					this.$api.addStorageFile.post({
						"Account-Token": this.statelocation.token,
						"associate_type":"FEEDBACK",
						"associate_uuid":this.Feedback_id,
						"suffix":"PNG",
						"file_base64":this.file_content,
					}).then((response)=>{
